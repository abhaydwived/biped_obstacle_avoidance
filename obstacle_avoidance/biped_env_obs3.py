...
class BipedWalkingEnv(gym.Env):
    ...
    # Motor control
    default_motor_force = 70.0
    pos_gain = 0.14
    vel_gain = 0.10

    def __init__(self, render=False, waypoints=None, seed: int = 123):
        super(BipedWalkingEnv, self).__init__()
        self.render = render
        ...
        # Velocity history for better stall detection
        self.velocity_history = deque(maxlen=int(0.5 / self.timestep))
        ...
        self.idx_L_knee = find_idx(["left", "knee"])
        self.idx_R_knee = find_idx(["right", "knee"])
        ...
        self.start_xy = np.array([0.0, 0.0])
        self.goal_xy = np.array([0.0, 6.0])
        ...
        self.left_contact_prev = False
        self.right_contact_prev = False

        # ----------------- METRICS (SR/SPL/CR/FR/CoT) -----------------
        self._g = 9.8
        self.robot_mass = float(p.getDynamicsInfo(self.robot_id, -1)[0])  # kg (base mass)
        self._episode_reset_metrics()

        self.reset()

    # -------------------- Metrics helpers --------------------
    def _compute_L_star_from_waypoints(self):
        """Approx shortest feasible path length from planned waypoint polyline."""
    ...
    def _episode_reset_metrics(self):
        """Reset per-episode accumulators."""
        self.ep_actual_path_len = 0.0
        self.ep_L_star = 0.0
        self.ep_collision_any = False
        self.ep_fall = False
        self.ep_mech_work = 0.0
        self.ep_steps = 0

        pos, _ = p.getBasePositionAndOrientation(self.robot_id)
        self._metric_prev_xy = np.array(pos[:2], dtype=np.float32)

    def get_episode_metrics(self):
        """Return paper metrics for the completed episode."""
        pos, _ = p.getBasePositionAndOrientation(self.robot_id)
        dist_goal = float(np.linalg.norm(self.goal_xy - np.array(pos[:2])))

        success = (dist_goal < 0.35) and (not self.ep_fall)

        L = float(self.ep_actual_path_len)
        Ls = float(self.ep_L_star)
        spl = (Ls / max(L, Ls)) if success and (max(L, Ls) > 1e-9) else 0.0

        denom = (self.robot_mass * self._g * max(L, 1e-9))
        cot = float(self.ep_mech_work / denom) if L > 1e-6 else float("inf")

        return {
            "success": bool(success),
            "spl": float(spl),
            "collision_any": bool(self.ep_collision_any),
            "fall": bool(self.ep_fall),
            "cot": float(cot),
            "L_actual": float(L),
            "L_star": float(Ls),
            "mech_work_J": float(self.ep_mech_work),
    ...
    def _astar(self, occ):
        ...
        si = np.clip(si, 0, nx - 1); sj = np.clip(sj, 0, ny - 1)
        gi = np.clip(gi, 0, nx - 1); gj = np.clip(gj, 0, ny - 1)
    ...
    def _spawn_obstacles(self):
        ...
        attempts = 0
        while len(self.obstacles) < self.num_obstacles and attempts < self.num_obstacles * 20:
            attempts += 1
    ...
    def _get_leg_separation(self):
        lf_pos = np.array(p.getLinkState(self.robot_id, self.left_foot_link)[0][:2])
        rf_pos = np.array(p.getLinkState(self.robot_id, self.right_foot_link)[0][:2])
        return float(np.linalg.norm(lf_pos - rf_pos))

    def _get_current_speed(self):
        lin_vel, _ = p.getBaseVelocity(self.robot_id)
        return math.hypot(lin_vel[0], lin_vel[1])
    ...
    def reset(self, seed=None, options=None):
        ...
        self._plan_path()
        self.ep_L_star = self._compute_L_star_from_waypoints()

        p.resetBasePositionAndOrientation(
            self.robot_id,
            [self.start_xy[0], self.start_xy[1], 0.45],
            p.getQuaternionFromEuler([0, 0, 0])
        )
        p.resetBaseVelocity(self.robot_id, [0, 0, 0], [0, 0, 0])

        # reset episode accumulators AFTER base reset so prev_xy is correct
        self._episode_reset_metrics()
        self.ep_L_star = self._compute_L_star_from_waypoints()

        pos, _ = p.getBasePositionAndOrientation(self.robot_id)
        self.prev_base_pos = pos
        self.last_check_pos = pos
        self.prev_distance_to_goal = np.linalg.norm(self.goal_xy - np.array(pos[:2]))
    ...
    def _compute_reward(self, target, tangent, cross_track):
        ...
        along_speed_reward = 6.5 * v_along
        facing_dir = np.array([np.cos(yaw), np.sin(yaw)])
        orientation_reward = 2.8 * float(np.dot(facing_dir, t))

        cross_penalty = -4.2 * (cross_track ** 2)

        upright_penalty = -2.0 * (abs(roll) + abs(pitch))
        height_penalty = -0.9 * abs(torso_z - 0.45)

        lateral_penalty = -2.5 * (v_lat ** 2)

        leg_sep = self._get_leg_separation()
        min_sep = 0.18
        leg_collision_penalty = 0.0
        if leg_sep < min_sep:
            leg_collision_penalty = -8.0 * ((min_sep - leg_sep) / min_sep) ** 2

        energy_penalty = -0.005 * sum(abs(p.getJointState(self.robot_id, i)[3]) for i in self.joint_indices)

        collision_penalty = -50.0 if self._check_collision() else 0.0

        goal_shaping_reward = 1.2 * (1.0 / (distance_to_goal + 1e-5))
        ...
        stuck_penalty = -15.0 if self.progress_stuck_counter > self.stuck_threshold_steps else 0.0

        low_velocity_penalty = -5.0 if v_along < 0.02 else 0.0
        ...
        swing_bonus = 0.0
        if not lc: swing_bonus += 0.5 * np.clip(lf_z - 0.03, 0.0, 0.03) / 0.03
        if not rc: swing_bonus += 0.5 * np.clip(rf_z - 0.03, 0.0, 0.03) / 0.03

        l_contact = len(p.getContactPoints(self.robot_id, self.plane_id, linkIndexA=self.left_foot_link)) > 0
        r_contact = len(p.getContactPoints(self.robot_id, self.plane_id, linkIndexA=self.right_foot_link)) > 0

        both_contact = l_contact and r_contact
        both_swing = (not l_contact) and (not r_contact)

        gait_quality = 0.0
        if both_contact and not (self.left_contact_prev and self.right_contact_prev):
            gait_quality = 0.3
        elif both_swing and (self.left_contact_prev or self.right_contact_prev):
            gait_quality = 0.2
        elif (l_contact and not r_contact and self.right_contact_prev and not self.left_contact_prev):
            gait_quality = 0.4
        elif (r_contact and not l_contact and self.left_contact_prev and not self.right_contact_prev):
            gait_quality = 0.4

        self.left_contact_prev = l_contact
        self.right_contact_prev = r_contact

        recovery_bonus = 1.5 if self.stuck_recovery_mode else 0.0
        ...
        reward = (
            ...
            leg_collision_penalty + energy_penalty + collision_penalty +
            goal_shaping_reward + stuck_penalty + time_penalty + low_velocity_penalty +
            swing_bonus + gait_quality + recovery_bonus
        )
    ...
    def step(self, action):
        ...
        pos_xy = np.array(base_pos[:2])

        # ---- METRIC: traveled distance ----
        self.ep_actual_path_len += float(np.linalg.norm(pos_xy - self._metric_prev_xy))
        self._metric_prev_xy = pos_xy.copy()

        # Look-ahead target + tangent
        ...
        v_des = float(np.clip(v_des, 0.08, 0.40))

        self.lookahead_dist = float(np.clip(0.6 + 1.2 * v_des / 0.40, 0.6, 1.6))

        # Stall detection
        current_speed = self._get_current_speed()
        self.velocity_history.append(current_speed)

        avg_speed = np.mean(list(self.velocity_history)) if len(self.velocity_history) > 0 else 0.0
        is_stuck = avg_speed < 0.05 and len(self.velocity_history) == self.velocity_history.maxlen
        ...
        if self.kick_stall_counter > self.anti_stall_steps:
            self._kick_until = self.step_counter + int(0.8 / self.timestep)
            self.stuck_recovery_mode = True
            self.stuck_recovery_steps = int(0.8 / self.timestep)
            self.kick_stall_counter = 0

        if self.stuck_recovery_mode:
            self.stuck_recovery_steps -= 1
            if self.stuck_recovery_steps <= 0:
                self.stuck_recovery_mode = False
        ...
        # ---- METRIC: mechanical work (approx) ----
        work_step = 0.0
        for jid in self.joint_indices:
            js = p.getJointState(self.robot_id, jid)
            qdot = float(js[1])
            tau = float(js[3])  # applied motor torque
            work_step += abs(tau * qdot) * self.timestep
        self.ep_mech_work += work_step

        p.stepSimulation()
        if self.render:
            time.sleep(self.timestep)

        # ---- METRIC: collision flag + step count ----
        self.ep_steps += 1
        if self._check_collision():
            self.ep_collision_any = True
        ...
        if self._check_fall():
            ...
            dist_goal = np.linalg.norm(self.goal_xy - np.array(current_pos[:2]))
            if dist_goal < 0.35:
                reward += 35.0
                terminated = True
            else:
        ...
        info = {
            ...
            "leg_separation": self._get_leg_separation(),
            "assist": self.assist_cpg,
            "lookahead": self.lookahead_dist,
            "stuck_recovery": self.stuck_recovery_mode,
            "avg_speed": np.mean(list(self.velocity_history)) if len(self.velocity_history) > 0 else 0.0,
        }

        # Optional: attach metrics at end of episode
